...
class Mongo_db
{
	...
	*/
	...
	public function or_where($wheres = array())
	{
	  ...
	  http://stackoverflow.com/questions/9640302/mongodb-query-filter-price
	 */

	public function elemMatch($wheres = array())
	{
		if (count($wheres) > 0)
		{
			if ( ! isset($this->wheres['$elemMatch']) OR ! is_array($this->wheres['$elemMatch']))
			{
				$this->wheres['$elemMatch'] = array();
			}

			$this->wheres['$elemMatch'][] = $wheres;
		}
		return $this;
	}
	...
	// Where field not exixts

	public function where_not_exist_field($field)
	{
		$this->_where_init($field);
		$this->wheres[$field]['$exists'] = null;
		return $this;
	}

	// Where field exists exixts

	public function where_exist_field($field)
	{
		$this->_where_init($field);
		$this->wheres[$field]['$exists'] = 1;
		return $this;
	}
	 ...
	 * @return object
	 */
	public function like_all($field = '', $value = '', $flags = 'i', $enable_start_wildcard = TRUE, $enable_end_wildcard = TRUE)
	{
		$field = (string) trim($field);
		$this->_where_init($field);
		$value = (string) trim($value);
	...
	*/
	public function get_one($collection = '')
	{
		if (empty($collection))
		{
			...
			// Clear
			$this->_clear($collection, 'get_one');
	...
	*/
	...
	public function update($collection = '', $options = array())
	{
		...
		try
		{
			...
			// echo "<pre>"; print_r($this->updates);echo "</pre>"; die();
	...
	public function create_dbref($collection = '', $field = '', $db_name = '')
	{
		...
		$database = ($db_name !== '') ? $db_name : $this->_dbname;
